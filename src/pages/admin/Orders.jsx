...
/* ─── Order status bar config ─── */
const ORDER_STATUSES = [
  { key: 'pending',         label: 'Pending',         color: '#f59e0b' },
  { key: 'confirmed',       label: 'Confirmed',        color: '#22c55e' },
  { key: 'processing',      label: 'Processing',       color: '#0066FF' },
...
function OrderStatusBar() {
  ...
  useEffect(() => {
      ...
      try {
        ...
        const results = await Promise.all(
          ORDER_STATUSES.map(s =>
            api.get(`/admin/orders?status=${s.key}&limit=1`)
              .then(r => ({ key: s.key, count: r.data.total ?? r.data.count ?? (r.data.orders?.length ?? 0) }))
    ...
    fetchCounts();
    // Refresh every 30 seconds for live accuracy
